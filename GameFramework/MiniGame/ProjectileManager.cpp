...
void ProjectileManager::HandleInput()
{
    if (InputHandler::Instance()->GetMouseButtonState(LEFT) || InputHandler::Instance()->IsKeyDown(SDL_SCANCODE_SPACE))
    {
        ...
        {
            Vector2D playerPos = player.GetPosition() + Vector2D(32.0f, 16.0f);
            ...
            missiles.emplace_back(std::make_unique<Missile>(LoaderParams(int(playerPos.GetX()) - 20, int(playerPos.GetY()), 63, 16, "missile"), angle));
